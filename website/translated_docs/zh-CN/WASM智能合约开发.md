...
本教程主要是指导用户在Alaya上使用wasm语言创建简单的HelloWorld智能合约，通过alaya-truffle编译，部署，调用此合约。
...
- alaya-truffle开发工具[安装参考](https://platon-truffle.readthedocs.io/en/alaya/getting-started/installation.html)
  ...
  如果wasm编译器因为网络问题总是下载失败，可以直接去github[链接地址](https://github.com/PlatONnetwork/PlatON-CDT/releases/download/v0.13.2/platon-cdt.tar.gz)下载编译好的压缩文件，然后解压到~/.config/alaya-truffle/compilers目录下，并对将platon-cdt/bin/wasm-opt软链接到/usr/bin目录
...
**step4.** 修改alaya-truffle 配置文件truffle-config.js，添加编译wasm合约版本号
...
2. 使用 `platon-cpp` 编译合约, 如何指定wasm输出目录及文件名?
    ...
    ```
3. alaya-truffle执行truffle deploy部署合约失败？
